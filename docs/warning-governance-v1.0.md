...
At this stage the engine has sealed a narrow `v1.0.0` contract.
...
At `v1.0.0`, the currently accepted warning is:
...
These should be treated as blockers for sustaining the `v1.0.0` claim:
...
- whether the project can keep its current `v1.0.0` release claim
- whether a warning requires a `v1.0.x` hotfix or a new hardening cycle
